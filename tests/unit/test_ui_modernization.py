...
class TestVisualGrouping:
    ...
    def test_primary_configuration_section_exists(self, html_soup):
        ...
        primary_config = html_soup.find(
            string=re.compile(r"Primary\s+Configuration", re.I)
        )
    ...
    def test_main_content_id(self, html_soup):
        ...
        ), "Main content element with id='main-content' not found"
...
class TestLiveInputValidation:
    ...
    def test_character_counter_constants(self, js_content):
        ...
        assert (
            "CHAR_COUNT_WARNING" in js_content
        ), "CHAR_COUNT_WARNING constant not found"
        ...
        assert (
            warning_match and int(warning_match.group(1)) == 4500
        ), "WARNING should be 4500"
    ...
    def test_character_counter_logic(self, js_content):
        ...
        has_input_listener = (
            "promptTextarea" in js_content and "addEventListener" in js_content
        ) or ("prompt" in js_content and "addEventListener" in js_content)
        ...
        assert (
            ".classList.add('valid')" in js_content
            or "classList.add('valid')" in js_content
        ), "Missing valid class logic"
        assert (
            ".classList.add('invalid')" in js_content
            or "classList.add('invalid')" in js_content
    ...
    def test_html_attributes_sync_documentation(self, html_content):
        ...
        assert (
            "synchronized with JavaScript constants" in html_content
            or "synced with JS constants" in html_content
            or "updated dynamically" in html_content
    ...
    def test_windows_reserved_names_validation(self, js_content):
        ...
        assert (
            "windowsReservedNames" in js_content
        ), "Missing Windows reserved names check"

        # Verify it includes correct reserved names (COM1-9, LPT1-9, not COM0/LPT0)
        reserved_pattern = r"com\[1-9\]|lpt\[1-9\]"
        assert re.search(
            reserved_pattern, js_content, re.I
        ), "Reserved names pattern incorrect (should be COM1-9, LPT1-9)"

        # Ensure COM0/LPT0 are NOT included
        assert (
            "[0-9]"
            not in re.search(r"windowsReservedNames.*?=.*?/(.*?)/", js_content).group(1)
            or "[1-9]" in js_content
        ), "Should use [1-9] not [0-9] for COM/LPT"

    def test_platform_detection(self, js_content):
        """Verify platform detection for path validation."""
        assert (
            "navigator.userAgentData" in js_content or "userAgentData" in js_content
    ...
    def test_colon_validation_logic(self, js_content):
        ...
        assert (
            "indexOf(':'," in js_content or "firstColonIndex" in js_content
        ), "Missing explicit colon index checking"

        # Should check for drive letter prefix
        assert "hasDriveLetterPrefix" in js_content or re.search(
            r"\[a-zA-Z\]:", js_content
    ...
    def test_empty_path_component_filtering(self, js_content):
        ...
        assert (
            ".filter(" in js_content and "length > 0" in js_content
    ...
    def test_validation_tooltips(self, js_content):
        ...
        has_title = (
            ...
            or ".title =" in js_content
            or 'setAttribute("title"' in js_content
        )
    ...
    def test_initial_validation_trigger(self, js_content):
        ...
        assert (
            "dispatchEvent(new Event('input'))" in js_content
    ...
    def test_form_submission_validation(self, js_content):
        ...
        assert (
            "classList.contains('invalid')" in js_content
        ), "Missing form submission check for invalid input"
        assert (
            "return;" in js_content or "preventDefault()" in js_content
...
class TestThemeSystem:
    ...
    def test_transition_properties(self, css_content):
        ...
        assert (
            "--transition-normal:" in css_content
            or "--transition-base:" in css_content
            or "--transition-medium:" in css_content
        ), "Missing normal/base transition"
    ...
    def test_form_section_opacity_property(self, css_content):
        ...
        assert (
            "--form-section-bg-opacity:" in css_content
    ...
    def test_error_shadow_property(self, css_content):
        ...
        assert (
            "--shadow-error:" in css_content
        ), "Missing --shadow-error custom property"
    ...
    def test_dark_and_light_theme_definitions(self, css_content):
        ...
        assert (
            ...
            or "[data-theme='light']" in css_content
    ...
    def test_theme_toggle_functionality(self, js_content):
        ...
        assert (
            "data-theme" in js_content or "setAttribute" in js_content
...
class TestMicroInteractions:
    ...
    def test_button_ripple_effect(self, css_content):
        ...
        assert (
            ".btn::before" in css_content or ".btn:before" in css_content
        ), "Missing button ripple pseudo-element"
        assert (
            "width: 200%" in css_content or "width:200%" in css_content
    ...
    def test_advanced_options_animation(self, css_content):
        ...
        assert "@keyframes slideDown" in css_content, "Missing slideDown keyframes"
        assert (
            "animation: slideDown" in css_content
            or "animation:slideDown" in css_content
    ...
    def test_toggle_icon_rotation(self, css_content):
        ...
        assert (
            "rotate(90deg)" in css_content or "rotate(90" in css_content
    ...
    def test_reduced_motion_support(self, css_content):
        ...
        if reduced_motion_section:
            ...
            has_animation_none = (
                "animation: none" in content or "animation:none" in content
            )
            has_transition_none = (
                "transition: none" in content or "transition:none" in content
            )
            assert (
                has_animation_none
            ), "Should fully disable animations for reduced motion"
            assert (
                has_transition_none
            ), "Should fully disable transitions for reduced motion"
    ...
    def test_ripple_disabled_for_reduced_motion(self, css_content):
        ...
        if reduced_motion_block:
            ...
            has_btn_before_disabled = (
                ".btn::before" in content or ".btn:before" in content
            ) and ("display: none" in content or "display:none" in content)
            assert (
                has_btn_before_disabled
            ), "Ripple effect should be disabled for reduced motion"
...
class TestAccessibility:
    ...
    def test_skip_link_z_index(self, css_content):
        ...
        skip_link_styles = re.search(
            r"\.skip-link.*?\{([^}]+)\}", css_content, re.DOTALL
        )
        assert skip_link_styles is not None, "Skip link styles not found"
        assert "z-index: 1000" in skip_link_styles.group(
            1
        ) or "z-index:1000" in skip_link_styles.group(
            1
    ...
    def test_aria_labels(self, html_soup):
        ...
        assert (
            len(elements_with_describedby) > 0
        ), "No aria-describedby attributes found"
    ...
    def test_aria_expanded(self, html_soup):
        ...
        if advanced_toggle:
            assert (
                advanced_toggle.get("aria-expanded") is not None
    ...
    def test_focus_visible_styles(self, css_content):
        ...
        focus_visible_rule = re.search(
            r"(\*|\w+)?:focus-visible\s*\{[^}]+outline", css_content, re.DOTALL
        )
        assert (
            focus_visible_rule is not None
    ...
    def test_form_input_font_size(self, css_content):
        ...
        input_styles = re.search(
            r"(input|textarea)[^{]*\{([^}]+)\}", css_content, re.DOTALL
        )
        if input_styles:
            styles = input_styles.group(2)
            assert (
                "font-size: 16px" in styles
                or "font-size:16px" in styles
                or "font-size: 1rem" in styles
    ...
    def test_error_state_focus_preservation(self, css_content):
        ...
        assert (
            ".invalid:focus" in css_content or "input.invalid:focus" in css_content
        ), "Missing invalid:focus styles to preserve error state"
        assert (
            ".valid:focus" in css_content or "input.valid:focus" in css_content
...
class TestResponsiveDesign:
    ...
    def test_flexible_layouts(self, css_content):
        ...
        assert (
            "display: grid" in css_content or "display:grid" in css_content
...
class TestCodeQuality:
    ...
    def test_no_unused_skeleton_classes(self, css_content):
        ...
        assert (
            ".skeleton" not in css_content
            or "@keyframes skeleton-loading" not in css_content
    ...
    def test_dom_reference_management(self, js_content):
        ...
        if "charCount" in js_content and "innerHTML" in js_content:
            ...
            assert "let charCount" in js_content or re.search(
                r"innerHTML.*?charCount\s*=\s*document", js_content, re.DOTALL
    ...
    def test_null_checks_for_dom_elements(self, js_content):
        ...
        assert (
            "?." in js_content or "if (" in js_content
        ...
        if "addEventListener" in js_content:
            assert (
                "?.addEventListener" in js_content or "if (" in js_content
    ...
    def test_constants_for_magic_numbers(self, js_content):
        ...
        validation_section = re.search(
            r"function.*?validateOutput.*?\{.*?\}", js_content, re.DOTALL
        )
        if validation_section:
            # Should not have literal 5000 or 10 in validation code
            assert "5000" not in validation_section.group(
                0
            ) or "CHAR_COUNT_MAX" in validation_section.group(
                0
    ...
    def test_platform_detection_comments(self, js_content):
        ...
        if "navigator.userAgent" in js_content:
            ...
            ua_section = re.search(
                r".{200}navigator\.userAgent.{200}", js_content, re.DOTALL
            )
            if ua_section:
                assert "//" in ua_section.group(0) or "/*" in ua_section.group(
                    0
...
class TestIntegration:
    ...
    def test_validation_feedback_channels(self, html_soup, js_content, css_content):
        ...
        has_feedback = (
            "title" in js_content
            or "aria-invalid" in js_content
            or "classList" in js_content
        )
    ...
    def test_theme_consistency(self, css_content):
        ...
        light_props = re.findall(
            r'\[data-theme=["\']light["\']\]\s*\{([^}]+)\}', css_content, re.DOTALL
        )
    ...
    def test_accessibility_cascade(self, html_soup, js_content):
        ...
        if "aria-expanded" in str(html_soup):
            assert (
                ...
                or '.setAttribute("aria-expanded"' in js_content
