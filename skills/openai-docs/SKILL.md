...
2. If it fails due to permissions or sandboxing, ask the user for explicit approval before retrying with escalated permissions (for example, sudo or admin privileges), and say exactly which command you will re-run.
3. If the user does not approve escalation, stop and provide the exact install command for the user to run manually.
4. If the escalated attempt still fails, ask the user to run the install command.
5. Ask the user to restart Codex.
6. Re-run the doc search/fetch after restart.
